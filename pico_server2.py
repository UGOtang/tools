...
<head>
    ...
    <style>
        ...
        h1 { color: #333; }
        #storage-info { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 12px; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(102,126,234,0.4); }
        #storage-info h3 { margin: 0 0 10px 0; font-size: 14px; opacity: 0.9; }
        #storage-bar { background: rgba(255,255,255,0.3); border-radius: 10px; height: 20px; overflow: hidden; }
        #storage-used { background: #fff; height: 100%; border-radius: 10px; transition: width 0.3s ease; }
        #storage-text { margin-top: 10px; font-size: 13px; opacity: 0.9; }
        ...
        #upload-status { margin-top: 15px; font-size: 14px; color: #666; }
        ...
        #file-list { list-style: none; padding: 0; background: #fff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); overflow: hidden; min-height: 60px;}
        ...
        .btn-delete { color: #f5222d; cursor: pointer; }
        .btn-delete:hover { text-decoration: underline; }
        .loading { color: #999; text-align: center; padding: 20px; }
        .error { color: #f5222d; text-align: center; padding: 20px; }
        .empty { color: #999; justify-content: center !important; }
    </style>
</head>
<body>
    <h1>📁 Pico 极速文件网盘</h1>
    <div id="storage-info">
        <h3>📊 存储空间</h3>
        <div id="storage-bar"><div id="storage-used" style="width: 0%"></div></div>
        <div id="storage-text">正在获取存储信息...</div>
    </div>
    <div id="drop-zone">
    ...
    <h2>文件列表</h2>
    <ul id="file-list"><li class="loading">加载中...</li></ul>
    <script>
        ...
        // 存储空间数据缓存
        let storageData = { total: 0, used: 0, free: 0 };
        ...
        function updateStorageDisplay() {
            if (storageData.total > 0) {
                const percent = (storageData.used / storageData.total * 100).toFixed(1);
                storageUsed.style.width = Math.min(percent, 100) + '%';
                storageText.textContent = `已用 ${formatSize(storageData.used)} / ${formatSize(storageData.total)}，剩余 ${formatSize(storageData.free)} 可用`;
            } else {
                storageText.textContent = '无法获取存储信息';
            }
        }
        ...
        async function handleFiles(files) {
            ...
            if (storageData.free > 0 && totalSize > storageData.free) {
                alert(`空间不足！需要 ${formatSize(totalSize)}，但只剩 ${formatSize(storageData.free)}`);
                return;
            }
            ...
            for (const file of files) {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        alert(`上传 "${file.name}" 失败: ${text || '服务器错误'}`);
                    }
                } catch (err) {
                    alert(`上传 "${file.name}" 网络错误: ${err.message}`);
                }
            }

            uploadBtn.disabled = false;
            uploadStatus.textContent = successCount > 0 ? `✅ 成功上传 ${successCount} 个文件` : '';
        ...
        async function deleteFile(filename) {
            ...
            if (!confirm(`确定要删除 "${decodedName}" 吗？`)) return;
        ...
        async function loadFileList() {
            ...
            try {
                ...
                if (!res.ok) {
                    fileList.innerHTML = '<li class="error">❌ 服务器错误，请刷新重试</li>';
                    return;
                }

                const data = await res.json();
                const files = data.files || data;  // 兼容新旧格式
                storageData = {
                    total: data.total || 0,
                    used: data.used || 0,
                    free: data.free || 0
                ...
                if (!files || files.length === 0) {
                    fileList.innerHTML = '<li class="empty">📭 网盘是空的，快去上传文件吧！</li>';
                } else {
                    fileList.innerHTML = files.map(f => `<li>
                        <span class="file-name">📄 ${f.name} <span style="color:#aaa; font-size: 12px;">(${formatSize(f.size)})</span></span>
                        <span class="file-actions">
                            <a href="/download?file=${encodeURIComponent(f.name)}" download class="btn-download">下载</a>
                            <a class="btn-delete" onclick="deleteFile('${encodeURIComponent(f.name)}'); return false;">删除</a>
                        </span>
                    </li>`).join('');
                }
            } catch (err) {
                console.error('加载文件列表失败:', err);
                fileList.innerHTML = '<li class="error">❌ 网络错误，请检查连接后刷新页面</li>';
            }
        }
...
def get_storage_info():
    """获取存储空间信息（MicroPython RP2040 兼容版本）"""
    used = 0
    try:
        for entry in os.ilistdir(UPLOAD_DIR):
            if entry[1] == 0x8000:  # 文件类型
                used += entry[3] if len(entry) > 3 else 0
    except:
        pass

    # RP2040 MicroPython 不支持 statvfs，使用估算值
    # Pico 有 2MB Flash，预留 512KB 给 MicroPython 固件和系统
    total_flash = 2 * 1024 * 1024  # 2MB
    system_reserve = 512 * 1024  # 512KB 系统预留
    usable = total_flash - system_reserve  # 可用空间约 1.5MB
    free = max(0, usable - used)
    return {'total': usable, 'used': used, 'free': free}
...
def serve_file_list(link_id):
    ...
    send_response(link_id, json.dumps(result), 'application/json')
